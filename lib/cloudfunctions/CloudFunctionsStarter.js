...
class CloudFunctionsStarter {
    ...
    install() {
        ...
        tb.packageJson.addNpmScript('serve', 'npm run build && firebase emulators:start --only functions');
        tb.packageJson.addNpmScript('shell', 'npm run build && firebase functions:shell');
        ...
        tb.copyAsset('src/graphql/index.ts', tb.stringToPath(`${tb.destination}/src/graphql`));
