...
class CloudFunctionsStarter {
    ...
    install(param) {
        ...
        param.framework.addNpmScript(param.packageJson, 'serve', 'npm run build && firebase emulators:start --only functions');
        param.framework.addNpmScript(param.packageJson, 'shell', 'npm run build && firebase functions:shell');
        param.framework.addNpmScript(param.packageJson, 'start', 'npm run shell');
        param.framework.addNpmScript(param.packageJson, 'deploy', 'firebase deploy --only functions');
        param.framework.addNpmScript(param.packageJson, 'logs', 'firebase functions:log');
        ...
        param.framework.addNpmScript(param.packageJson, 'build:watch', 'tsc --watch');
        ...
        param.framework.addNpmScript(param.packageJson, 'test', 'jest --colors --detectOpenHandles');
        param.framework.addNpmScript(param.packageJson, 'ci-test', 'npm run test -- --collectCoverage --reporters=default --reporters=jest-junit --ci');
        param.framework.mkdir(path.normalize(`${param.destination}/src`));
        param.framework.mkdir(path.normalize(`${param.destination}/src/test`));
        copyAsset('src/test/helloWorld.test.ts', path.normalize(`${param.destination}/src/test`));
        ...
        param.framework.addNpmScript(param.packageJson, 'prettier', "prettier --check --write '**/*.{ts,js,json,md}'");
        param.framework.addNpmScript(param.packageJson, 'lint', "eslint '**/*.ts' -f codeframe --fix");
        param.framework.addNpmScript(param.packageJson, 'codestyle', 'npm run prettier && npm run lint');
        param.framework.addNpmScript(param.packageJson, 'ci-lint', 'npm run lint -- -f checkstyle -o ./output/checkstyle-result.xml');
        copyAsset('src/config.ts', path.normalize(`${param.destination}/src`));
        copyAsset('src/logger.ts', path.normalize(`${param.destination}/src`));
        copyAsset('src/index.ts', path.normalize(`${param.destination}/src`));
        param.framework.mkdir(path.normalize(`${param.destination}/src/graphql`));
        copyAsset('src/graphql/index.ts', path.normalize(`${param.destination}/src/graphql`));
        ...
        function copyAsset(name, destination) {
            param.framework.cpFile(param.asset(path.normalize(name)), destination);
        }
    }
}
